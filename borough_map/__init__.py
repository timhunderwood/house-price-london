"""
1) Shape file for london:
Shape file for London is

Statistical GIS Boundary Files for London
Greater London Authority (GLA)
https://data.london.gov.uk/dataset/statistical-gis-boundary-files-london

statistical-gis-boundaries-london.zip
containing London_Borough_Excluding_MHW.shp which is used.

https://s3-eu-west-1.amazonaws.com/londondatastore-upload/Geography-licensing.pdf

On maps created using these boundaries the copyright must be stated. This is: "Contains National Statistics data © Crown copyright and database right [2015]" and "Contains Ordnance Survey data © Crown copyright and database right [2015]"
Contains National Statistics data © Crown copyright and database right 2012
Contains Ordnance Survey data © Crown copyright and database right 2012

2) Price paid data UK

https://www.gov.uk/government/statistical-data-sets/price-paid-data-downloads
http://www.nationalarchives.gov.uk/doc/open-government-licence/version/3/

Contains HM Land Registry data © Crown copyright and database right 2019. This data is licensed under the Open Government Licence v3.0.

Download full single file as CSV

"""
